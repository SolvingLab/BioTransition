...
CorandPval <- function(x, y = NULL, use = "everything",
  ...
  if (ia == "two.sided") {
    tstat <- sqrt(np - 2) * abs(cor) / sqrt(1 - cor^2)
    p <- 2 * stats::pt(tstat, np - 2, lower.tail = FALSE)
  } else if (ia == "less") {
    tstat <- sqrt(np - 2) * cor / sqrt(1 - cor^2)
    p <- stats::pt(tstat, np - 2, lower.tail = TRUE)
  } else if (ia == "greater") {
    tstat <- sqrt(np - 2) * cor / sqrt(1 - cor^2)
    p <- stats::pt(tstat, np - 2, lower.tail = FALSE)
  }
