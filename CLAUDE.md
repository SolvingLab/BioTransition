...
**v2.1.0 已清（勿回退）：** nCores 小核机变负数 → `max(1, cores-1)`；全方法进度 `cat()` → `message()`（可被 `suppressMessages()` 静默）；`1:n` → `seq_len/seq_along`；空态 / 缺 `ref` / 空 PPI 统一走 `validateInput` 友好报错；大基因数聚类栈溢出 → 可操作提示；CI 统一 |PCC| 口径；cDNB/tDNB 评分向量化提速 ~19×。
...
- **`fast_module_score_cpp` 是低效死代码**：全矩阵按值拷贝 + O(n²·N) is-outside 线性扫描，实测比 `scoreModules` 的向量化 R 慢 ~50×，无人调用。建议删除或重写。
- **LDNB 边方向 `rbind`**：把 (A,B) 与翻转的 (B,A) `rbind`，若上游已含双向边会重复计数——需核对语义。
- **TOM 分母为 0**：`calSFNetforCorMatrix` 的 `numTOM/denomTOM` 在某基因连通度为 0 时可能 `Inf`，无保护，极端输入下 `hclust` 会崩。
